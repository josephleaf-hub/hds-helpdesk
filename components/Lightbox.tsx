...
export function LightboxProvider({ children }: { children: React.ReactNode }) {
  ...
  const open = useCallback((url: string, name?: string) => setImg({ url, name }), []);
    ...
    <LightboxCtx.Provider value={open}>
      ...
      {img && (
        <div className="lightbox-overlay" onMouseDown={close}>
          ...
          <img className="lightbox-img" src={img.url} alt={img.name || 'attachment'} onMouseDown={(e) => e.stopPropagation()} />
        </div>
      )}
